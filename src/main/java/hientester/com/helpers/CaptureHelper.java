...
public class CaptureHelper extends ScreenRecorder {
    ...
    public static void startRecord(String methodName) {
        ...
        File file = new File(SystemHelper.getCurrentDir() + PropertiesHelper.getValue(ConstantGlobal.RECORD_VIDEO_PATH));//"Video_Record_Path"));
    ...
    public static void captureScreenshot(String screenshotName) {
        try {
            ...
            File theDir = new File(SystemHelper.getCurrentDir() + PropertiesHelper.getValue(ConstantGlobal.SCREENSHOT_PATH));//"Screenshot_Path"));
            if (!theDir.exists()) {
                theDir.mkdirs();
            }
            // Chổ này đặt tên thì truyền biến "screenName" gán cho tên File chụp màn hình
            FileHandler.copy(source, new File(SystemHelper.getCurrentDir() + PropertiesHelper.getValue(ConstantGlobal.SCREENSHOT_PATH) + File.separator + screenshotName + "_" + dateFormat.format(new Date()) + ".png"));
    ...
    public static void takeScreenshot(String screenshotName){
       ...
       File thDir = new File("./screenshots");
        ...
        //Note: Tên pải trùng với tên đặt ở dòng 20
        try {
